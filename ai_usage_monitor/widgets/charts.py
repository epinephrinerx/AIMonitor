...
class StackedColumnChart(QWidget):
    ...
    def paintEvent(self, event) -> None:  # noqa: N802 - Qt naming
        ...
        # Value labels on the caps. Only where the text actually fits the band
        # and the day has usage - a label that collides with its neighbour or
        # sits over an empty slot is worse than no label at all.
        painter.setBrush(Qt.BrushStyle.NoBrush)
        label_font = QFont(self.font())
        label_font.setPointSizeF(max(6.5, self.font().pointSizeF() - 1.0))
        ...
        for index, bucket in enumerate(self.buckets):
            if bucket.total <= 0:
                continue
            text = formatting.metric_label(bucket.total, self.metric_name)
            text_width = label_metrics.horizontalAdvance(text)
            if text_width > band_width - 2:
                continue  # would run into the next column
            centre = plot.left() + band_width * (index + 0.5)
            if centre - text_width / 2 < drawn_until + 4:
                continue  # would touch the label before it
            top = plot.bottom() - plot.height() * (bucket.total / axis_max)
            cap = top - label_metrics.height() - 2
            if cap < plot.top():
                cap = top + 2  # no room above the cap; sit just inside it
            painter.setPen(qcolor(self.theme.ink_secondary))
            painter.drawText(
                QRectF(centre - band_width / 2, cap, band_width, label_metrics.height()),
                Qt.AlignmentFlag.AlignCenter,
                text,
            )
            drawn_until = centre + text_width / 2
