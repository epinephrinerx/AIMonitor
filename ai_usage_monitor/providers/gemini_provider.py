...
class GeminiProvider(Provider):
    ...
    key_placeholder = r"C:\path\to\service-account.json  (optional)"
    extra_label = "Google Cloud project id"
    extra_placeholder = "auto-detected from your login if left blank"
    tagline = "Gemini CLI · Cloud Monitoring"
    setup_hint = (
        "Detected automatically from your <b>Gemini CLI</b> login if you have "
        "signed in with <code>gemini</code> — that token already carries the "
        "cloud-platform scope this needs.<br><br>"
        "Otherwise point this at a Google Cloud <b>service account JSON</b> "
        "with the <b>Monitoring Viewer</b> role.<br><br>"
        "Either way, usage comes from <b>Cloud Monitoring</b>, because Google "
        "publishes no usage endpoint for Gemini. Gemini Advanced "
        "<i>subscription</i> limits are not available from any public API."
    )
    ...
    def sources(self) -> list[Source]:
        """The shared list, with `manual` bound to the saved key file."""
        saved = self._path
        return bind(
            GEMINI_SOURCES,
            "manual",
            (lambda: self._service_account_credential(saved)) if saved else (lambda: None),
        )
    ...
    def _bearer(self, credential: Credential) -> str:
        """An access token, however the credential was obtained.

        A service account is exchanged for one via a signed JWT; the Gemini CLI
        already holds one with cloud-platform scope, so it is used directly. We
        never refresh the CLI's token - that is its own business.
        """
        if credential.kind == SERVICE_ACCOUNT:
            return self._access_token(self._load_key_file(credential.value))
        if credential.kind == OAUTH and credential.value:
            return credential.value
        raise _GeminiError("No usable Google credential was found.")

    def _discover_project(self, token: str) -> str:
        """Ask Cloud Resource Manager which project this login can see."""
        request = urllib.request.Request(
            "https://cloudresourcemanager.googleapis.com/v1/projects"
            "?filter=lifecycleState:ACTIVE&pageSize=50",
            method="GET",
        ...
        if not projects:
            raise _GeminiError(
                "This Google account has no active Cloud project. Create one "
                "and enable the Generative Language API, then set the project "
                "id in Settings."
            )
        if len(projects) > 1:
    ...
    def fetch(self, days: int, metric: str, want_history: bool) -> ProviderSnapshot:
        ...
        snapshot = ProviderSnapshot(
            provider_id=self.id,
            configured=detected.usable,
            detection=detected,
            setup_hint=self.setup_hint,
            value_note=(
                "Request counts from Cloud Monitoring; Google reports no "
                "token-level cost here."
            ),
        )
        credential = detected.credential
        if not detected.usable or credential is None:
            if detected.state == "expired":
                snapshot.error = detected.hint
            return snapshot

        try:
            token = self._bearer(credential)
            project = self._project or credential.project or self._discover_project(token)
        ...
        who = credential.account or "signed in"
    ...
    def _load_key_file(self, path: str) -> dict:
        try:
            data = json.loads(Path(path).read_text(encoding="utf-8"))
