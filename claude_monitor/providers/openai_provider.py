...
class OpenAIProvider(Provider):
    ...
    setup_hint = (
        "Needs an OpenAI <b>Admin</b> key (Settings → Organization → Admin keys) "
        "from an owner account with the Usage Dashboard permission. A regular "
        "sk-… project key will not work.<br><br>"
        "This reports <b>API platform</b> usage and spend. ChatGPT Plus/Pro "
        "message limits are not available from any public API."
    )
    ...
    def fetch(self, days: int, metric: str, want_history: bool) -> ProviderSnapshot:
        ...
        snapshot.account = "OpenAI organization (API platform)"
