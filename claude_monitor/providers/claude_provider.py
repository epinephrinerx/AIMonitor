...
from .. import api, credentials, formatting, throughput, usage_log
from .base import (
    HistoryView,
    Meter,
    Provider,
...
class ClaudeProvider(Provider):
    ...
    def __init__(self) -> None:
        ...
        self._throughput = throughput.ThroughputTracker()
    ...
    def set_throughput_interval(self, seconds: int) -> None:
        self._throughput.set_interval(seconds)
    ...
    def fetch(self, days: int, metric: str, want_history: bool) -> ProviderSnapshot:
        ...
        if want_history:
            ...
            # Throughput only advances here: widget mode parses no transcripts,
            # so there is no token total to diff. A long spell in widget mode
            # shows up as one long interval, correctly averaged.
            self._throughput.observe(self._store.all_totals.output_tokens)
            snapshot.throughput = ThroughputView(
                samples=self._throughput.series(),
                peak=self._throughput.peak,
                current=self._throughput.current,
                mean=self._throughput.mean,
                pending_tokens=self._throughput.pending_tokens,
                interval_seconds=self._throughput.interval_seconds,
            )
