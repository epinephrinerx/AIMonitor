...
@unittest.skipUnless(secrets.available(), "sealed key storage needs Windows or macOS")
class ClearAndCancelTests(unittest.TestCase):
    ...
    def _wipe(self):
        # Clear through the API, not just the settings store: on macOS the
        # stored value is a reference and the secret itself lives in the
        # Keychain, so dropping the settings alone would leave one test item
        # behind per run.
