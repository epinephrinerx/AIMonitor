...
### เลขเวอร์ชันของ release v1.1.0 — เก็บไว้ อย่าไปแก้ประวัติ

release `v1.1.0` คือ build macOS ตัวแรก ผู้ใช้สั่งให้เก็บไว้เป็นบันทึก
ทุกอย่างที่ผู้ใช้มองเห็นบอก 1.1.0 ตรงกันหมด ทั้ง `CFBundleShortVersionString`
ชื่อไฟล์ `.dmg` และ README ของ commit นั้น

**ยกเว้นจุดเดียว**: `__init__.__version__` ที่ commit `af83887` เขียนว่า `2.0.0`
ค่านี้ค้างมาตั้งแต่ commit แรกของ repo และตอนนั้นไม่มีอะไรอ่านมันเลย เพราะสาขา `Mac`
แยกไปก่อนที่ `version.py` กับ About box จะเกิด จึงไม่เคยหลุดไปถึงผู้ใช้
**อย่าไปแก้ tag หรือเขียนประวัติใหม่เพื่อให้มันตรง** ของที่แจกไปแล้วถูกต้องอยู่
และ `test_version.py` คุมตั้งแต่ 1.3.1 เป็นต้นไป ซึ่งเป็นรุ่นแรกที่ฝั่ง macOS
อ่านเลขจาก `__init__` แทนการเขียนไว้เอง

release นั้นมีหมายเหตุบนหัวเรื่องแล้วว่าถูกแทนที่ด้วย 1.3.1 เพื่อไม่ให้ใครโหลดผิดรุ่น
ตั้งแต่ 1.3.1 ไป **ไฟล์ .dmg ไปอยู่ใน release เดียวกับ .exe** ไม่แยก release ต่อแพลตฟอร์ม
